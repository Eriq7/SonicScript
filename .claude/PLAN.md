...
## Long-Press Translation Mode [x]

Replace unreliable keyword-based translation with deterministic long-press gesture.

- [x] HotkeyManager: strict state machine — double-tap cancels long-press, long-press suppresses double-tap
- [x] types.ts: add HOTKEY_LONG_PRESS IPC, remove TranslationSettings + translation field
- [x] constants.ts: remove translation block from DEFAULT_SETTINGS.llm
- [x] index.ts: wire long-press callback to send HOTKEY_LONG_PRESS
- [x] preload/index.ts: add onHotkeyLongPress, update startRecording(translate?)
- [x] FloatingWidget.tsx: shared beginRecording(), translateMode state, yellow UI
- [x] ipc-handlers.ts: re-entry guard (sessionActive check), pass translate flag to processWithLLM
- [x] llm-processor.ts: translate param, throw when LLM unavailable for translation
- [x] translation.ts: remove detectAndStripTrigger, keep buildTranslationPrompt
- [x] LLMSettings.tsx: remove keyword editor + translation toggle, add long-press hint
- [x] HotkeyConfig.tsx: "Double-tap to Record" text + long-press hint in footer
- [x] store.ts: remove translation migration block
