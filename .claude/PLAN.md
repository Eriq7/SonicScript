...
**根因 2（长语音截断）**：SFSpeechRecognizer 内部分段时触发 `isFinal=true`，Swift 的 `cleanupTask()` + Electron 的 `.once('final')` 联合终止了整个 session，50% 启发式是死代码。

**修复**：
- [x] `src/renderer/index.html`：删除 Google Fonts 三行 `<link>` 标签
- [x] `src/main/speech/SonicScriptHelper.swift`：新增 `stoppingByRequest` + `accumulatedText`；内部 `isFinal` 时积累文本并重启识别任务（不停音频引擎）；用户停止时 emit 完整累积文本；partial 输出包含前缀
